...
<head>
    ...
    <title>Subir Expediente Final - CMN <?= defined('ANIO_CMN') ? ANIO_CMN : date('Y') ?> | REGPOL LIMA</title>
    ...
    <style>
        :root {
            ...
            --accent-color: #d32f2f;       /* Rojo para Consolidación */
            --bg-color: #f0f2f5;
...
<body>
    <div class="premium-topbar">
        <div class="topbar-logo">
            ...
            (iii) FASE DE CONSOLIDACIÓN Y APROBACIÓN
        </div>
    </div>
    ...
    <div class="container">
        <div class="main-card shadow-lg">
            <div class="card-header-gradient">
                <h2 class="fw-bold mb-1 animate__animated animate__fadeInDown" style="letter-spacing: -0.5px;">SUBIR EXPEDIENTE FINAL (CONSOLIDACIÓN Y APROBACIÓN)</h2>
                    ...
                    <div class="alert alert-danger rounded-4 p-4 border-0 shadow-sm animate__animated animate__shakeX">
                        <div class="d-flex align-items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="mb-0">El periodo para la carga del Expediente Final ha concluido o la fase se encuentra bloqueada momentáneamente.</p>
                            </div>
                        </div>
                    </div>
                <?php else: ?>
                    <form id="formCmnAnexo" action="../controlador/cmn_anexo_fase3_save.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="info-alert mb-4 d-flex align-items-center">
                            ...
                            <div>
                                ...
                                Usted debe subir el <strong>Expediente Final (Consolidado y Aprobado): Cuadro Multianual de Necesidades - Fase de Consolidación</strong> debidamente <strong>firmado, sellado y visado por el Director o Jefe de la Unidad Ejecutora</strong> en formato PDF.
                            </div>
                        </div>
                        ...
                        <div id="uploadZone" class="upload-section-disabled">
                            <div class="section-title mt-5">Adjuntar Expediente Final</div>
                                ...
                                <div class="col-12">
                                    <div class="form-check mb-3 p-3 border rounded bg-warning bg-opacity-10 border-warning">
                                        ...
                                        <label class="form-check-label fw-bold text-dark" for="checkFirmas">
                                            <i class="fa-solid fa-check-double me-1 text-success"></i> DECLARO QUE MI PDF CUENTA CON TODAS LAS FIRMAS Y SELLOS DEL EXPEDIENTE FINAL.
                                        </label>
                                    </div>
                                    ...
                                    ">
                                        ...
                                        <div id="fileInfo">
                                            <h5 class="mb-1 fw-bold" id="fileNameDisplay">Haga clic aquí para seleccionar el archivo PDF (Expediente Final)</h5>
                        ...
                        <div class="mt-5 text-center upload-section-disabled" id="submitZone">
                            <button type="submit" class="btn btn-main-submit w-100">
                                <i class="fa-solid fa-cloud-arrow-up me-2"></i> FINALIZAR Y ENVIAR CONSOLIDACIÓN
                            </button>
                        </div>
                    </form>
                <?php endif; ?>
    ...
    <script>
        ...
        function buscarDNI() {
            ...
            if(dni.length === 8) {
                ...
                fetch('../controlador/cmn_get_data_dni.php?dni=' + dni + '&fase=3')
                    .then(r => r.json())
                    .then(data => {
                        Swal.close();
                        if(data.exists) {
                            if(data.has_annex) {
                                ...
                                Swal.fire({
                                    icon: 'warning',
                                    title: 'EXPEDIENTE YA REMITIDO',
                                    html: `<div class="text-start mt-2" style="font-size: 0.88rem;">
                                            <p class="mb-2">El <b>Expediente Final de Consolidación</b> de su Sub Unidad ya fue enviado.</p>
                                ...
                                Swal.fire({ icon: 'success', title: '¡Validado!', text: 'Puede subir el Expediente Final.', timer: 1500, showConfirmButton: false });
                            }
                            ...
                            Swal.fire('No habilitado', 'Este DNI no está registrado en el padrón de responsables <?= defined('ANIO_CMN') ? ANIO_CMN : date('Y') ?>.', 'error');
                        }
                    });
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            <?php if(isset($_SESSION['msg_status'])): ?>
                Swal.fire({
                    ...
                    title: '<?= $_SESSION['msg_status'] === 'success' ? '¡Éxito!' : 'Error' ?>',
