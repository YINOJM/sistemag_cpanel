        ...
        <div class="container-fluid">
            ...
            <div class="premium-header">
                ...
                <div class="d-flex gap-2">
                    ...
                    <a href="../controlador/reporte_cmn_excel.php?q=<?= urlencode($search_query) ?>&anio=<?= $anio ?>" class="btn-modern btn-modern-success">Excel</a>
                    <a href="../controlador/cmn_generar_reporte_pdf.php?q=<?= urlencode($search_query) ?>&anio=<?= $anio ?>" target="_blank" class="btn-modern btn-modern-danger">Reporte PDF</a>
                </div>
            </div>
                    ...
                    <form action="" method="GET" class="row g-2 align-items-center" id="formFiltros">
                        ...
                        <div class="col-md-1">
                            <select name="anio" class="form-select form-select-sm" onchange="this.form.submit()">
                                <option value="<?= $anio_actual ?>" <?= $anio == $anio_actual ? 'selected' : '' ?>><?= $anio_actual ?></option>
                                <option value="<?= $anio_actual - 1 ?>" <?= $anio == ($anio_actual - 1) ? 'selected' : '' ?>><?= $anio_actual - 1 ?></option>
                            </select>
                        </div>
                        <div class="col-md-1">
                                    ...
                                    <tr id="row-<?= $row['id'] ?>" class="<?= $clase_row ?>">
                                        ...
                                        <td class="text-center align-middle">
                                            <div class="d-flex justify-content-center gap-1">
                                                <button type="button" class="btn-action <?= $est === 2 ? 'btn-validado' : 'btn-pdf' ?>" onclick="verPdfModal('<?= $row['id'] ?>', '../uploads/cmn_<?= $anio_actual ?>/<?= $row['archivo_pdf'] ?>', '<?= addslashes($row['apellidos']) ?>', <?= $est ?>)" title="Ver Solicitud PDF"><i class="fa-solid <?= $est === 2 ? 'fa-check-double' : 'fa-eye' ?>"></i></button>
   ...
   const m = "🚨 *OBSERVACION CMN* 🚨\n\nEstimado/a *" + nombre.toUpperCase() + "*,\n\nSe le informa que hemos recibido su documento de la *Fase de Identificación*; sin embargo, se ha observado que el archivo presenta inconsistencias como *firmas y/o sellos faltantes* (Responsable Logístico y Jefe de Unidad) o errores en el llenado del documento.\n\nPor favor, sírvase revisar detalladamente su archivo y volver a cargarlo con todos los requisitos completos. Quedamos atentos a la nueva versión.";
    ...
    Swal.fire({
        ...
        function copiarLinkPublico() {
            const link = window.location.origin + "/sistem_job/cmn.php";
