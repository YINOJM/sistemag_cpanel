<?php
// controlador/ren_generar_notificacion_pdf.php
ob_start();
...
// 2. Obtener datos de la rendición
...
// 4. Conversión de Imágenes a Base64 para PDF
...
// 5. Preparar datos para el PDF (Limpios de PNP y puntos extras)
...
$siaf_nro = $row['siaf_expediente'] ?: 'SIN_SIAF';

// 5.5 Cargar Plantilla desde BD
...
$intro_text = applyTemplate($vals['intro']  ?? '', $data_map);
$item1_text = applyTemplate($vals['item1']  ?? '', $data_map);
$item2_text = applyTemplate($vals['item2']  ?? '', $data_map);
$item3_text = applyTemplate($vals['item3']  ?? '', $data_map);
$outro_text = applyTemplate($vals['outro']  ?? '', $data_map);

// 6. HTML del PDF - DISEÑO PREMIUM
$html = '
<!DOCTYPE html>
<html>
<head>
    ...
    <style>
        ...
        /* Watermark */
        .watermark {
            position: fixed; top: 25%; left: 15%; width: 70%; opacity: 0.05; z-index: -1000;
        }
...
<body>
    ...
    <div class="justify content">' . $intro_text . '</div>

    <ol>
        <li>' . $item1_text . '</li>
        <li><i>' . $item2_text . '</i></li>
        <li>' . $item3_text . '</li>
    </ol>

    <div class="justify">' . $outro_text . '</div>
...
ob_end_clean();
$dompdf->stream("Notificacion_SIAF_".$siaf_nro.".pdf", ["Attachment" => false]);
?>
